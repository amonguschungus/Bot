...
module.exports = class PlayCommand extends Command {
    ...
    async run(message, { query }) {
        ...
        if (!voiceChannel) return message.say('You cant listen to music if you are not in a call :smile:');
        // end initial check
        ...
        if (query.match(/^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+/)) {
            ...
            try {
                ...
                console.error(err);
                return message.say('Sorry, something happened, and I dont know what.');
            }
        }
        try {
            ...
            const embed = new MessageEmbed()
                .setColor('#e9f931')
                .setTitle('Please choose a number between 1-5 :smile:')
            ...
            try {
                ...
                return message.say(
                    'Sorry, the only option here is to have a number 1 through 5 :smile:'
                );
            }
